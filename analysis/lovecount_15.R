...
## Data
# source("analysis/lyr_analysis_15.R")
load("data/mello15_all.Rdata")

## Atomic words ----
lyrics <- lapply(mello15_all$lyrics, function(lyric) {
...
for (i in 1:nrow(mello15_all)) {
  if (lovecount[i] < 0) {
    ...
    phrase_matches <- str_count(mello15_all$lyrics[i] %>% tolower(), lovephrases) %>% sum()
  }
...
mello15_all$lovecount <- lovecount
mello15_all$lovebins <- lovebins
...
# l <- mello15_all %>% filter(lovecount == 0)
...
mello15_all$seasoncount <- seasoncounts
mello15_all$seasonbins <- seasonbins
...
mello15_all$godcount <- godcounts
mello15_all$godbins <- godbins
...
mello15_all$partycount <- partycounts
mello15_all$partybins <- partybins
